...
int main(int argc, char **argv)
{
    ...
    std::thread thread2(move_to_retract_position, base_R);
    thread1.join();
    thread2.join();
    ...
    cartesian_action_movement(base_R, base_cyclic_R, desired_pose_delta_R);
    
    std::this_thread::sleep_for(std::chrono::milliseconds(2000));

    fix_wrench_R = get_measured_wrench(base_cyclic_R);
    fix_wrench_L = get_measured_wrench(base_cyclic_L);
    ...
    LowPassFilter lpf1_L(2.0, 0.02);  /// low-pass filter
    LowPassFilter lpf2_L(2.0, 0.02);
    LowPassFilter lpf3_L(2.0, 0.02);
    ...
    {   
        ...
        current_pose_L = get_measured_toolpose(base_cyclic_L);
        current_vel_L = get_measured_toolvel(base_cyclic_L);
        current_wrench_L = get_measured_wrench(base_cyclic_L);

        
        for(int i = 0; i < 6; i++)
        {
            // diff_wrench_L[i] = current_wrench_L[i] - fix_wrench_L[i];
        ...
        diff_wrench_L[0] = lpf1_L.update(diff_wrench_L[0]);
        diff_wrench_L[1] = lpf2_L.update(diff_wrench_L[1]);
        diff_wrench_L[2] = lpf3_L.update(diff_wrench_L[2]);    


        std::vector<float> new_pose_R = controller.calculateAdmitPose(current_pose_R, current_vel_R, diff_wrench_R, loop_time);
        std::vector<float> new_pose_L = controller.calculateAdmitPose(current_pose_L, current_vel_L, diff_wrench_L, loop_time);
        ...
        thread1 = std::thread(run_twist_command,base_cyclic_R, base_R, new_pose_R, loop_ms);
        thread2 = std::thread(run_twist_command,base_cyclic_L, base_L, new_pose_L, loop_ms);

        thread1.join();
        thread2.join();

        usleep(20000); // Sleep for u second
    }
