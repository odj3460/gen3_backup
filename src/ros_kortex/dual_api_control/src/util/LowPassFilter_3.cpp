...
LowPassFilter_3::LowPassFilter_3():
	output(0),
	ePow(0){}

LowPassFilter_3::LowPassFilter_3(float iCutOffFrequency, float iDeltaTime):
	output(0),
...
float LowPassFilter_3::update(float input){
	return output += (input - output) * ePow;
}

float LowPassFilter_3::update(float input, float deltaTime, float cutoffFrequency){
	reconfigureFilter(deltaTime, cutoffFrequency); //Changes ePow accordingly.
	return output += (input - output) * ePow;
}

void LowPassFilter_3::reconfigureFilter(float deltaTime, float cutoffFrequency){
	#if ERROR_CHECK
	if (deltaTime <= 0){
		std::cout << "Warning: A LowPassFilter instance has been configured with 0 s as delta time.";
		ePow = 0;
